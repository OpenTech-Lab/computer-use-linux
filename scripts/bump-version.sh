...
INIT_PY="${PROJECT_ROOT}/src/computer_use_linux/__init__.py"
...
Arguments:
  <version>   Semantic version to release (e.g. 0.2.0)
...
# __init__.py is the single source of truth. pyproject.toml declares
# dynamic = ["version"] and reads this attribute, and cli.py's --version imports it, so one
# edit propagates to the package metadata and the CLI alike. Guard that arrangement rather
# than editing several files and hoping they stay in step.
read_init_version() {
  grep -m1 '^__version__ = ' "${INIT_PY}" | sed -E 's/__version__ = "([^"]+)"/\1/'
}

CURRENT_VERSION="$(read_init_version)"

if [ -z "${CURRENT_VERSION}" ]; then
  echo "Error: could not read __version__ from ${INIT_PY}"; exit 1
fi

if ! grep -q 'dynamic = \["version"\]' "${PYPROJECT}"; then
  echo "Error: ${PYPROJECT} no longer declares dynamic = [\"version\"]."
  echo "It must read the version from ${INIT_PY}, or releases will silently ship a stale"
  echo "number in the package metadata while the CLI reports the new one."
  exit 1
fi
...
if ! echo "${VERSION}" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$'; then
  echo "Error: version must be in format x.y.z (e.g. 0.2.0)"; exit 1
fi
...
# Fail loudly if the edit silently missed; a half-applied bump is worse than none.
if [ "$(read_init_version)" != "${VERSION}" ]; then
  echo "Error: version edit did not apply cleanly to ${INIT_PY}"; exit 1
fi
...
# Commit
git -C "${PROJECT_ROOT}" add "${INIT_PY}" "${README}"
