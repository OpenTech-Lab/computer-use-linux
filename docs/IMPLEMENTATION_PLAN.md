...
| Blender | **5.1.2 at `/opt/blender/blender`** | Launch with `--python <extras/blender-addon/cul_bridge.py>`; the addon opens a TCP socket on `127.0.0.1:9876` and executes submitted Python on Blender's main thread via `bpy.app.timers`. This mirrors the blender-mcp addon pattern already in use on this machine. Actions: `run_python`, `scene_info`, `viewport_screenshot`. |
| Godot | 4.6.2.stable at `~/.local/bin/godot` | Two modes: headless (`godot --headless --script res://...`) for scripted runs, and an `EditorPlugin` in `extras/godot-plugin/addons/cul_bridge/` opening a TCP socket for live editor control. Actions: `run_scene`, `eval_gdscript`, `editor_command`. |
